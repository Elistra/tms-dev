...
function find_file_by_uid($id) {
    ...
    $file_name = mysqli_query($connection, $query);
...
function find_user_files() {
    ...
    $query  = "SELECT users.username, stock.name ";
    $query .= "FROM users, stock";
    $query .= "WHERE stock.user_id = users.id";
    $query .= "ORDER BY username ASC";
    $user_set = mysqli_query($connection, $query);
    confirm_query($user_set);
    return $user_set;
	...
	function find_admin_by_username($username) {
		...
		$query .= "LIMIT 1";
		$admin_set = mysqli_query($connection, $query);
        // until now you get all data about the data, in this case "bob", dev
		confirm_query($admin_set);
		if($admin = mysqli_fetch_assoc($admin_set)) {
			return $admin;
		} else {
	...
	function confirm_logged_in() {
		if (!logged_in()) {
			redirect_to("clientloginpage.php");
		}
	}

    function confirm_admin() {
        if (!logged_in_admin()) {
            redirect_to("clientloginpage.php"); // Change this to a user page
        }
    }
